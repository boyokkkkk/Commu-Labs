...
%% ======================== 画图 ============================
figure;
subplot(2,2,1); imshow(img_original);        title('原始图像');
subplot(2,2,2); imshow(img_unipolar_nrz);    title(['单极性NRZ  BER=' num2str(BER_unipolar_nrz,'%.4f')]);
subplot(2,2,3); imshow(img_bipolar_nrz);     title(['双极性NRZ  BER=' num2str(BER_bipolar_nrz,'%.4f')]);
subplot(2,2,4); imshow(img_diff);            title(['差分码      BER=' num2str(BER_diff,'%.4f')]);